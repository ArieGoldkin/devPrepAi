/**
 * JSON Parser Service
 * Handles robust JSON parsing with multiple fallback strategies
 */

import type { IQuestion } from "@/types/ai";
import { ASSESSMENT_DEFAULTS } from "@lib/store/constants";

/**
 * Parse Claude API response with multiple fallback strategies
 */
export function parseClaudeResponse(textContent: string): { questions: IQuestion[] } {
  // Try primary parsing
 ...
 */
function tryPrimaryParsing(textContent: string): { questions: IQuestion[] } | null {
  try {
 ...
 */
function trySecondaryParsing(textContent: string): { questions: IQuestion[] } | null {
  try {
    ...
    if (arrayMatch !== null && arrayMatch !== undefined && arrayMatch[1] !== null && arrayMatch[1] !== undefined) {
 ...
 */
function getValidType(type: unknown, defaultType?: IQuestion['type']): IQuestion['type'] {
  const validTypes = ['coding', 'system-design', 'behavioral', 'conceptual'] as const;
