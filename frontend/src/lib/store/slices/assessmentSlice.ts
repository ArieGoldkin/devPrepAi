...
export interface IAssessmentActions {
  startAssessment: (
    questions: IQuestion[],
    settings?: Partial<IAssessmentState["settings"]>,
  ) => void;
...
> = (set, get) => ({
  ...
  startAssessment: (
    ...
    set({
      ...
      timeRemaining:
        (settings?.duration ?? ASSESSMENT_DEFAULTS.duration) *
  ...
  submitAnswer: (questionId: string, answer: string): void =>
    set((state) => {
      const existingAnswer = state.answers.find(
        (a: IAssessmentAnswer) => a.questionId === questionId,
      );
      ...
      const timeSpent = Math.floor(
        (currentTime - startTime) / TIME_CONSTANTS.MS_PER_SECOND,
      );
      ...
      const updatedAnswers =
        existingIndex >= 0
          ? state.answers.map((a: IAssessmentAnswer, i: number) =>
  ...
  completeAssessment: (): IAssessmentResults => {
    ...
    const questionResults: IQuestionResult[] = state.questions.map(
      (question: IQuestion) => {
        const answer = state.answers.find(
          (a: IAssessmentAnswer) => a.questionId === question.id,
        );
        return {
          question,
    ...
    const totalTimeAllocated =
      state.settings.duration * TIME_CONSTANTS.SECONDS_PER_MINUTE;
  ...
  resetAssessment: (): void => set(initialAssessmentState),
...
): IQuestion | null => state.questions[state.currentQuestionIndex] ?? null;
...
export const selectTimeProgress = (state: IAssessmentState): number => {
  ...
  return totalTime > 0
    ? ((totalTime - state.timeRemaining) / totalTime) * 100
    : 0;
};
