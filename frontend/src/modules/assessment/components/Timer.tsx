...
import { TIME_CONSTANTS, UI_THRESHOLDS } from "@lib/store/constants";
...
export function Timer({
  ...
  const getColorClass = (): string => {
    ...
    if (percentRemaining <= UI_THRESHOLDS.TIME_CRITICAL_PERCENT)
      return "text-red-600";
    if (percentRemaining <= UI_THRESHOLDS.TIME_WARNING_PERCENT)
  ...
  return (
    <div
      className={`font-mono text-headline font-bold ${getColorClass()} ${className}`}
    >
      ...
      <div className="text-caption text-gray-500 mt-1">
        {Math.floor(timeRemaining / TIME_CONSTANTS.SECONDS_PER_MINUTE)}m{" "}
        {timeRemaining % TIME_CONSTANTS.SECONDS_PER_MINUTE}s remaining
      </div>
    </div>
  );
}
