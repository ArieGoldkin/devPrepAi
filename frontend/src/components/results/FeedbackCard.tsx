...
const getIconColor = (colorClass: string): string =>
  colorClass.replace("text-", "text-").replace("-700", "-500");
    ...
    <CardHeader>
      <div className="flex items-center justify-between">
        ...
        <div className="text-right">
          <div
            className={`inline-flex items-center px-3 py-1 rounded-full text-white text-sm font-medium ${scoreColorClass}`}
          >
            {feedback.score}/100
          </div>
    ...
    <div>
      ...
      <ul className="space-y-1">
        {items.map((item, index) => (
          ...
          >
            <span className={`mr-2 mt-0.5 ${iconColorClass}`}>{icon}</span>
            {item}
          </li>
        ))}
      </ul>
