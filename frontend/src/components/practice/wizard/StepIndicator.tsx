...
export function StepIndicator({ currentStep }: IStepIndicatorProps): React.JSX.Element {
  ...
  const renderStepIcon = (stepNumber: number, _isActive: boolean, isCompleted: boolean): React.ReactNode => {
    if (isCompleted) {
  ...
  return (
    <div className="flex items-center justify-between mb-8 max-w-2xl mx-auto">
      {['Welcome', 'Profile', 'Settings', 'Ready'].map((step, index) => {
        ...
        return (
          <React.Fragment key={step}>
            ...
            {index < TOTAL_STEPS - 1 && (
              <div className={`flex-1 h-0.5 mx-2 ${isCompleted ? 'bg-brand-success' : 'bg-muted'}`} />
            )}
          </React.Fragment>
        )
      })}
