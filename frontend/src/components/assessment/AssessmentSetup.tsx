...
const saveToStorage = (settings: IAssessmentSettings): void => {
  if (typeof window !== "undefined") {
    localStorage.setItem("devprep-assessment-settings", JSON.stringify(settings))
  }
}
...
export function AssessmentSetup({ onComplete, onBack }: IAssessmentSetupProps): React.JSX.Element {
  ...
  const handleSubmit = (): void => {
    if (timerDuration && questionCount && difficultyLevel) {
      const settings: IAssessmentSettings = { timerDuration, questionCount, difficultyLevel }
  ...
  const isFormValid = Boolean(timerDuration && questionCount && difficultyLevel)
